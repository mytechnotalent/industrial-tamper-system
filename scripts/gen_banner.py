#!/usr/bin/env python3
"""Generate the industrial-tamper-system.png banner (1500x1500, stdlib).
...
def _draw_titles(banner, colors) -> None:
    ...
    None
    """
    banner.text("INDUSTRIAL TAMPER SYSTEM", 4,
                colors["gold"], banner.W // 2, 90)
    banner.text("OPERATION IRON WEB", 10, (0x0B, 0x10, 0x26),
                banner.W // 2 - 6, 176)
    banner.text("OPERATION IRON WEB", 10, colors["white"],
                banner.W // 2, 182)
    banner.text("RP2350  PICO 2", 5, colors["cyan"], banner.W // 2, 560)
    banner.text("-=[ MESH DEFENSE + PROPAGATION LAB ]=-", 3, colors["dim"],
                banner.W // 2, 660)
...
def _draw_features(banner, colors) -> None:
    ...
    None
    """
    banner.text("INDUSTRIAL_TAMPER_SYSTEM", 5, colors["cyan"],
                banner.W // 2, 750)
    banner.text("LATCH  ARM BUTTON", 4, colors["silver"],
                banner.W // 2, 900)
    banner.text("IR REMOTE  DHT11 CABINET", 3, colors["silver"],
                banner.W // 2, 990)
    banner.text("XCHACHA20-POLY1305  ARGON2ID", 4, colors["dim"],
                banner.W // 2, 1120)
    banner.text("WORM  PROPAGATION", 4, colors["gold"],
                banner.W // 2, 1260)
    banner.text("SEALED ALERT  FAIL-SAFE", 4, colors["gold"],
                banner.W // 2, 1350)
    banner.text("-=[ OPERATION IRON WEB  /  RP2350 ]=-", 3, colors["dim"],
                banner.W // 2, 1440)
...
def main() -> int:
    ...
    out = Path(__file__).resolve().parent.parent / "industrial-tamper-system.png"
    banner.save(out)
