 ...
 *
 * DESCRIPTION:
 * Native unit tests for the RP2350 IRON LUNG HVAC automation node
 * status LED, manual arm button, latch servo, infrared decoder,
...
static void assert_lamps(int red, int yellow, int green) {
    ...
    TEST_ASSERT_EQUAL_INT(yellow,
                          mock_gpio_get(TAMPER_YELLOW_LED_PIN));
    TEST_ASSERT_EQUAL_INT(green,
                          mock_gpio_get(TAMPER_GREEN_LED_PIN));
}
 ...
 * @return void
 */
static void assert_show(tamper_led_state_t state, int red, int yellow,
                        int green) {
...
static void test_status_led_show(void) {
    ...
    assert_show(TAMPER_OFF, 0, 0, 0);
    assert_show(TAMPER_INTRUSION, 1, 0, 0);
    assert_show(TAMPER_ARMED, 0, 1, 0);
    assert_show(TAMPER_SECURE, 0, 0, 1);
}
 ...
 * @return void
 */
static void test_arm_pressed(void) {
    reset_pc();
    TEST_ASSERT_TRUE(arm_init());
    gpio_put(TAMPER_BUTTON_PIN, true);
    TEST_ASSERT_FALSE(arm_pressed());
    gpio_put(TAMPER_BUTTON_PIN, false);
    TEST_ASSERT_TRUE(arm_pressed());
}
 ...
 * @return void
 */
static void test_arm_consume(void) {
    reset_pc();
    arm_init();
    gpio_put(TAMPER_BUTTON_PIN, true);
    TEST_ASSERT_FALSE(arm_consume_press());
    gpio_put(TAMPER_BUTTON_PIN, false);
    TEST_ASSERT_TRUE(arm_consume_press());
    TEST_ASSERT_FALSE(arm_consume_press());
}
 ...
 * @return void
 */
static void test_arm_debounce(void) {
    reset_pc();
    arm_init();
    gpio_put(TAMPER_BUTTON_PIN, false);
    TEST_ASSERT_TRUE(arm_consume_press());
 ...
 * @return void
 */
static void test_arm_debounce_elapsed(void) {
    reset_pc();
    arm_init();
    gpio_put(TAMPER_BUTTON_PIN, false);
    TEST_ASSERT_TRUE(arm_consume_press());
    arm_release();
    mock_timer_set_us(mock_timer_now_us() + ARM_DEBOUNCE_US);
    gpio_put(TAMPER_BUTTON_PIN, false);
    TEST_ASSERT_TRUE(arm_consume_press());
}

/**
 * @brief Verify arm_reset clears the debounce state.
 *
 * @param void No parameters.
 * @return void
 */
static void test_arm_reset(void) {
    reset_pc();
    arm_init();
    gpio_put(TAMPER_BUTTON_PIN, false);
    TEST_ASSERT_TRUE(arm_consume_press());
...
static void test_servo_init(void) {
    ...
    slice = pwm_gpio_to_slice_num(TAMPER_SERVO_PIN);
...
static void test_ir_init(void) {
    ...
    TEST_ASSERT_FALSE(s_mock_gpio_dirs[TAMPER_IR_PIN]);
}
...
static void run_indicator_tests(void) {
    ...
    RUN_TEST(test_arm_pressed);
    RUN_TEST(test_arm_consume);
    RUN_TEST(test_arm_debounce);
    RUN_TEST(test_arm_debounce_elapsed);
    RUN_TEST(test_arm_reset);
}
